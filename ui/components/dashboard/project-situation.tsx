...
export function ProjectSituation({ projects, onEdit, onDelete, limit }: { projects: DashboardProjectSituation[]; onEdit?: (identifier: string) => void; onDelete?: (identifier: string) => void; limit?: number }) {
  ...
  const visibleProjects = limit === undefined ? projects : projects.slice(0, limit);

  return (
    <div>
      <div className="hidden grid-cols-[minmax(180px,1.6fr)_110px_110px_110px_120px_120px] border-b bg-[hsl(var(--muted)/0.45)] px-5 py-2.5 text-center text-[11px] font-medium text-muted-foreground lg:grid">
        <span>项目</span><span>测试资产</span><span>最近执行</span><span>运行任务</span><span>最近活动</span><span>操作</span>
      </div>
      <div className="divide-y">
        {visibleProjects.map((project) => {
          const risk = riskMeta[project.risk_level];
          return (
            <div key={project.identifier} className="grid gap-3 px-4 py-4 transition-colors hover:bg-[hsl(var(--muted)/0.35)] lg:grid-cols-[minmax(180px,1.6fr)_110px_110px_110px_120px_120px] lg:items-center lg:px-5 lg:py-3">
              ...
              <div className="flex justify-center gap-1">
                <Link href={`/projects/${project.identifier}`} className="inline-flex h-8 items-center gap-1 whitespace-nowrap rounded-lg px-2 text-xs font-medium text-[hsl(var(--primary))] hover:bg-[hsl(var(--accent))]">
