...
async def rag_query_data(
    ...
    参数：
        ...
        mode: 检索模式。可选值：
              ...
              - naive（朴素向量检索）
              - bypass（直接转发LLM，不使用知识库）
        top_k: 检索的实体/关系数量（默认25，快速检索可保持默认）
        chunk_top_k: 检索的文本块数量（默认3）
        enable_rerank: 是否启用重排序以提高检索质量（默认False）
        enable_vlm_enhanced: 是否启用VLM视觉增强查询（默认False，勿随意开启）
...
async def rag_document_status(
    ...
    """Return LightRAG document indexing pipeline status."""
    client = _get_client(ctx)
    try:
        result = await client.get("/documents/pipeline_status", space_id=space_id)
        return json.dumps(result, ensure_ascii=False, indent=2)
        ...
        logger.exception("rag_document_status failed")
        return json.dumps({"status": "unreachable", "error": str(e)}, ensure_ascii=False)
...
async def rag_health(
    ...
    """Check whether the configured LightRAG API service is reachable."""
    client = _get_client(ctx)
    try:
        result = await client.get("/health")
        return json.dumps({"status": "healthy", "detail": result}, ensure_ascii=False, indent=2)
    except RAGAPIError as e:
        return json.dumps({"status": "unhealthy", "error": str(e)}, ensure_ascii=False)
    except Exception as e:
        return json.dumps({"status": "unreachable", "error": str(e)}, ensure_ascii=False)
...
def main():
    ...
    if args.transport == "sse":
        port = int(os.environ.get("RAG_MCP_PORT", args.port))
