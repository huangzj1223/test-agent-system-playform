// eslint-disable  MC80OmFIVnBZMlhwdTRUbGphRG1zWjg2WVc1QmJnPT06M2Y5M2VlN2E=

import { test, expect } from '@playwright/test';
// FIXME  MS80OmFIVnBZMlhwdTRUbGphRG1zWjg2WVc1QmJnPT06M2Y5M2VlN2E=
...
// TODO  Mi80OmFIVnBZMlhwdTRUbGphRG1zWjg2WVc1QmJnPT06M2Y5M2VlN2E=
...
test.describe('POST /pet - Add a new pet to the store', () => {
  ...
  test('TC-04: 缺少必填字段 name（观察行为）', async ({ request }) => {
    // Petstore API 即使缺少 name 也会返回 200（自动填充或容忍）
    const invalidPet = {
      id: generatePetId(),
    ...
    const body = await response.json();
    console.log(`响应内容: ${JSON.stringify(body)}`);

    // API 容忍缺失 name，但不应返回 5xx
    expect(response.status()).toBeLessThan(500);
  });

  test('TC-05: 发送空对象请求体（观察行为）', async ({ request }) => {
    // Petstore API 对空对象 {} 会返回 200（自动填充默认值）
    // 这是一个观察性测试，记录 API 的实际行为
    test.slow();
    ...
    if (response.status() === 200) {
      const body = await response.json();
      console.log(`服务端接受了空对象，返回数据: ${JSON.stringify(body)}`);
      // 验证返回结构包含基本字段
  ...
  test('TC-06: 使用无效的 status 枚举值（观察行为）', async ({ request }) => {
    // Petstore API 接受任何 status 值（不强制枚举校验）
    const petData = createTestPet({
      name: '无效状态宠物',
    ...
    if (response.status() === 200) {
      const body = await response.json();
      console.log(`服务端接受了无效枚举值，返回 status: ${body.status}`);
      expect(body.status).toBe('unknown_status_value');
    } else {
      const body = await response.text();
      console.log(`响应内容: ${body}`);
    }
...
// eslint-disable  My80OmFIVnBZMlhwdTRUbGphRG1zWjg2WVc1QmJnPT06M2Y5M2VlN2E=
