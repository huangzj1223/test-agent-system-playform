// NOTE  MC80OmFIVnBZMlhwdTRUbGphRG1zWjg2TW01V1VnPT06YTI5NTdhMTg=

import { test, expect } from '@playwright/test';

const BASE_URL = 'https://petstore.swagger.io/v2';
// FIXME  MS80OmFIVnBZMlhwdTRUbGphRG1zWjg2TW01V1VnPT06YTI5NTdhMTg=

// 测试辅助函数：生成唯一的宠物 ID
function generatePetId(): number {
  return Math.floor(Math.random() * 100000) + Date.now();
}
// NOTE  Mi80OmFIVnBZMlhwdTRUbGphRG1zWjg2TW01V1VnPT06YTI5NTdhMTg=
...
test.describe('POST /pet - Add a new pet to the store', () => {
  ...
  test('TC-04: 缺少必填字段 name（观察行为）', async ({ request }) => {
    ...
    // API 可能接受此请求，我们记录行为但只验证请求不会导致 5xx 错误
    expect(response.status()).toBeLessThan(500);
  });

  test('TC-05: 发送空对象请求体', async ({ request }) => {
    // 使用 test.slow() 给这个测试更多时间
    test.slow();
...
// TODO  My80OmFIVnBZMlhwdTRUbGphRG1zWjg2TW01V1VnPT06YTI5NTdhMTg=
